...
def phiEx(tValues, parameters, initialConditions):
    ...
    # Calcular los valores de phiRhs y phiPhase
    phiRhsValues = [
        calculatePhiRhs(zetak, zetaell, omega1, omega3, k, ell, t) for t in tValues
    ]
    phiPhase = phaseFred(phiRhsValues)

    # Calcular el resultado de phiEx
    phiExResult = [0.5 * phase for phase, t in zip(phiPhase, tValues)]
    return phiExResult
...
def psiEx(tValues, parameters, initialConditions):
    ...
    psiRhsValues = [
        calculatePsiRhs(zetak, zetaell, omega1, omega3, k, ell, t) for t in tValues
    ]

    # Calcular la fase de psi
    psiPhase = phaseFred(psiRhsValues)

    # Calcular el resultado de psiEx
    psiExResult = [
        0.5 * phase - (c * (b - a)) / (a * b) * t
        for phase, t in zip(psiPhase, tValues)
    ]
